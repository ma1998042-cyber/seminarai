...
import {
  ...
  Type, AlignLeft, CheckSquare, Circle, ChevronDown, Star, Hash, CreditCard, Mail, FileText, X,
} from "lucide-react";
import { cn, SURVEY_CATEGORY_LABELS } from "@/lib/utils";
import { createSurvey, getEmailTemplatesForNew } from "./actions";
...
const mkQuestion = (type: QuestionType, order: number): Question => ({
...
export default function NewSurveyForm({
  ...
  const [completionEmailSubject, setCompletionEmailSubject] = useState("");
  const [completionEmailBody, setCompletionEmailBody] = useState("");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">アンケートを作成</h1>
          <p className="text-sm text-gray-500">設問を追加してアンケートフォームを作成してください</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="アンケートの目的や回答者へのメッセージ"
          />
        </div>
        <div>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              onClick={async () => {
                ...
                const result = await getEmailTemplatesForNew();
                setTemplates(result);
      ...
      <div className="space-y-4">
          ...
          <div key={question.id} className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
                placeholder="設問のタイトルを入力してください"
              />
                ...
                <div className="space-y-2 pl-4">
                  {question.options.map((option, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      />
                      <button onClick={() => removeOption(question.id, i)} className="text-gray-300 hover:text-red-400 transition-colors">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ))}
                  <button onClick={() => addOption(question.id)} className="text-sm text-indigo-600 hover:underline flex items-center gap-1">
        ...
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-6">
            ...
            {questionTypes.map((type) => (
              <button
                ...
                className={cn("flex items-center gap-1.5 px-3 py-2 rounded-lg border text-xs font-medium transition-all",
                  selectedType === type.value ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-200 text-gray-600 hover:border-gray-300"
                )}
              >
          ...
          <button
            onClick={() => setQuestions([...questions, mkQuestion(selectedType, questions.length)])}
      ...
      <div className="flex gap-3">
        <Link href="/surveys" className="flex-1 py-3 rounded-lg border border-gray-200 text-gray-600 text-sm font-medium text-center hover:bg-gray-50 transition-colors">
          キャンセル
        </Link>
        <button onClick={() => handleSubmit("draft")} disabled={loading} className="flex-1 py-3 rounded-lg border border-indigo-200 text-indigo-600 text-sm font-semibold hover:bg-indigo-50 disabled:opacity-50 transition-colors">
          下書き保存
        </button>
        <button onClick={() => handleSubmit("active")} disabled={loading} className="flex-1 bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">
          ...
          <ClipboardList className="w-4 h-4" />公開する
        </button>
      </div>
    </div>
  );
