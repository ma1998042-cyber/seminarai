...
export default function CaseStudyEditForm({ caseStudy }: Props) {
  const router = useRouter();
  const [title, setTitle] = useState(caseStudy.title);
  const [imageUrl, setImageUrl] = useState(caseStudy.imageUrl);
  const [productionPeriod, setProductionPeriod] = useState(caseStudy.productionPeriod);
  const [productionCost, setProductionCost] = useState(caseStudy.productionCost);
  const [description, setDescription] = useState(caseStudy.description);
  const [sortOrder, setSortOrder] = useState(caseStudy.sortOrder);
  const [isPublished, setIsPublished] = useState(caseStudy.isPublished);
  ...
  async function handleSave() {
    ...
    const result = await updateCaseStudyAction(caseStudy.id, {
      title,
      imageUrl,
      productionPeriod,
      productionCost,
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">事例を編集</h1>
        </div>
        <span
