...
import { createOrganization, createEvent, completeOnboarding, acceptInvitationAndCompleteOnboarding } from "./actions";
...
export default function OnboardingClient({ pendingInvitations }: { pendingInvitations: PendingInvitation[] }) {
  ...
  // Step 1: Organization
  const [orgMode, setOrgMode] = useState<"create" | "join" | null>(
    pendingInvitations.length > 0 ? null : "create"
  );
  ...
  const handleAcceptInvitation = async (token: string) => {
    ...
    const result = await acceptInvitationAndCompleteOnboarding(token);
            ...
            <div className="space-y-6">
              <div className="text-center mb-6">
                ...
                <h1 className="text-xl font-bold text-gray-900">
                  {pendingInvitations.length > 0 ? "組織を選択" : steps[0].title}
                </h1>
              ...
              {pendingInvitations.length > 0 && (
                <div className="space-y-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setOrgMode(orgMode === "create" ? null : "create")}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Building2 className={`w-5 h-5 ${orgMode === "create" ? "text-indigo-600" : "text-gray-400"}`} />
                      <span className={`text-sm font-semibold ${orgMode === "create" ? "text-indigo-700" : "text-gray-700"}`}>
                        新しい組織を作成する
                      </span>
                    </div>
                  </button>
              ...
              {/* Mode selection (when no invitations) */}
              {pendingInvitations.length === 0 && (
              ...
              {orgMode === "create" && (
                <form onSubmit={handleCreateOrg} className="space-y-6">
                  {pendingInvitations.length === 0 && error && (
                    <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600">
                      {error}
                    </div>
                  )}
              ...
              {/* Join existing organization (no invitations case) */}
              {orgMode === "join" && pendingInvitations.length === 0 && (
                <div className="space-y-6">
